...
function extractMetricFromLinesWithRegex(
  ...
  for (const line of lines) {
    ...
    const displayValue = match[0].replace(/\s+/g, " ").trim();
    const numericValue = parseMetricNumber(match[1] ?? displayValue);
    if (numericValue === null) {
      continue;
    }
...
function rulesProposalHasContent(proposal: PlanningRulesProposal): boolean {
  return (
    typeof proposal.max_height_m.value === "number" ||
    typeof proposal.max_floors.value === "number" ||
    typeof proposal.buildability_m2_m2.value === "number" ||
    typeof proposal.occupancy_percent.value === "number" ||
    typeof proposal.setbacks.front_m.value === "number" ||
    typeof proposal.setbacks.rear_m.value === "number" ||
    typeof proposal.setbacks.side_m.value === "number" ||
    proposal.uses_allowed.values.length > 0 ||
    proposal.uses_forbidden.values.length > 0
  );
}
...
export function applyPlanningExtractionProposal(
  ...
  if (rulesProposalHasContent(result.rulesProposal)) {
    appliedFields.push("planning.rules_proposal");
  }
  ...
  const reviewNotes = appendText(planning.review_notes, [
    ...
    rulesProposalHasContent(result.rulesProposal)
      ? "Se han generado propuestas revisables en planning.rules_proposal. Acepta, edita o rechaza cada regla antes de confirmar la normativa."
